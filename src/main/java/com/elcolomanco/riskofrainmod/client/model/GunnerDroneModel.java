...
public class GunnerDroneModel <T extends GunnerDroneEntity> extends EntityModel<T> {
    ...
    public GunnerDroneModel() {
        ...
        this.top_case_front_1_right.setPos(3.0F, 1.5F, -4.5F);
        ...
        this.top_case__front_1_left.setPos(-3.0F, 1.5F, -4.5F);
        ...
        this.lower_case_left.setPos(-4.5F, 0.0F, 0.0F);
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.eye.xRot = 0;
		//Looking rotations
		this.gun_pitch_axis.xRot = headPitch * ((float)Math.PI / 180F);
    	this.gun_yaw.yRot = netHeadYaw * ((float)Math.PI / 180F);
    	//Propeller
    	if(entityIn.isTame() && entityIn.isFlying() && !entityIn.isInSittingPose()) {
    		float f = ageInTicks * (float)Math.PI * 0.3F;
    		this.eye.y = (MathHelper.cos(ageInTicks * 0.18F) * 0.9F);
    		this.propeller_rod.yRot = f;
    	} else if (entityIn.isOnGround()) {
    		this.eye.y = 0;
    		this.propeller_rod.yRot = 0.7853981633974483F;
    	}
    	//Buried position
    	if (this.bodyPitch > 0.0F && !entityIn.isTame()) {
    	...
    	} else if (entityIn.isOnGround() && entityIn.isTame()) {
    		this.core.y = 18.75F;
    	} else {
    		this.core.y = 15.0F;
    	}
